...
class FarmvilleDiv2:
    def minTime(self, time, cost, budget):
        pairs = sorted(zip(time, cost), key = lambda x:x[1])
...
def do_test(t, cost, budget, __expected):
    startTime = time.time()
    instance = FarmvilleDiv2()
    exception = None
    try:
        __result = instance.minTime(t, cost, budget);
    except:
        import traceback
...
def run_tests():
    ...
    with open("FarmvilleDiv2.sample", "r") as f:
        while True:
            ...
            passed += do_test(t, cost, budget, __answer)

    sys.stdout.write("\nPassed : %d / %d cases\n" % (passed, cases))

    T = time.time() - 1450465068
    PT, TT = (T / 60.0, 75.0)
